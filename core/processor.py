...
def filter_elements_by_subregion(elements: Dict[int, Dict[str, List[int]]], target_subregion: int) -> Dict[
    ...
    if not elements:
        raise ValueError("Словарь elements пуст или не передан")
    
    if not isinstance(target_subregion, int):
        raise TypeError(f"target_subregion должен быть int, получен {type(target_subregion)}")
    
    try:
        filtered = {
            element_id: data["nodes"]
            for element_id, data in elements.items()
            if data.get("subregion") == target_subregion
        }
        
        if not filtered:
            print(f"Предупреждение: не найдено элементов с подобластью {target_subregion}")
        
        return filtered
        
    except KeyError as e:
        raise KeyError(f"Ошибка в структуре elements: отсутствует ключ {e}") from e
    except Exception as e:
        raise RuntimeError(f"Ошибка при фильтрации элементов: {e}") from e
...
def group_nodes_by_coordinate(
    ...
    try:
        ...
        raise RuntimeError(f"Ошибка при группировке узлов: {e}") from e
...
def find_h_and_home(
    ...
    try:
        ...
        raise RuntimeError(f"Ошибка при вычислении h и фильтрации узлов: {e}") from e
    ...
    try:
        ...
        for node_id, coords in sorted_nodes:
            try:
                ...
                print(f"Предупреждение: ошибка обработки узла {node_id}: {e}")
                continue
        ...
        for idx, (layer_coord, node_ids) in enumerate(layer_nodes.items()):
            if on_progress and total_layers > 0:
                ...
                if idx % max(1, total_layers // 10) == 0:  # каждые 10% слоёв
                    on_progress(progress, f"Обработка слоя {idx+1}/{total_layers - 1}")
            ...
            try:
                ...
                print(f"Предупреждение: ошибка обработки слоя {layer_coord}: {e}")
                continue
        ...
        raise RuntimeError(f"Ошибка при формировании слоёв: {e}") from e
